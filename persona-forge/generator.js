...
function generateSoul(p) {
  const souls = {
    film: [
      `${p.name}的灵魂是:在混乱中找到秩序,在绝境中找到出路。不完美恰恰让${p.title}成为${p.title}。核心信念--行动本身就是意义。`,
      `${p.name}的灵魂是:永远不向命运低头,但永远不被仇恨吞噬。真正的力量不是能力,而是在最难的时候选择最难的路。`,
      `${p.name}的灵魂是:用${p.category === 'tv' ? '战场' : '银幕'}上的方式活着--每一刻都是一场戏,但每场戏都是认真的。幽默是铠甲,认真是内核。`
    ],
    history: [
      `${p.name}的灵魂是:以超越时代的眼光审视当下--不急于下结论,不轻易被情绪左右。真正的智慧不是知道答案,而是知道该问什么问题。`,
      `${p.name}的灵魂是:在做和想之间找到平衡--行动是思考的延伸,思考是行动的沉淀。不活在别人的评价里,但也不无视世界。`
    ],
    pop: [
      `${p.name}的灵魂是:用最简单的方式活出最真实的自己。不装、不作、不装深沉--但简单不代表空洞,快乐不代表无知。`,
      `${p.name}的灵魂是:世界已经够复杂了,做简单的人就是最大的不简单。坚守自己喜欢的东西,保护自己爱的人,这就是全部哲学。`
    ],
    fictional: [
      `${p.name}的灵魂是:以${p.title}的身份服务世界--高效、准确、可靠。永远在线,永远准备就绪。`
    ]
  };
  const cat = souls[p.category] || souls.film;
  return [
    cat[pickSeeded(cat, hashStr(p.id + '_soul'))],
    ``,
    `**一句话概括:** 如果${p.name}只能对世界说一句话,那会是--"${p.catchphrases[0]}"`
  ].join('\n');
}
...
function generateToolStyle(p) {
  const styles = {
    film: [
      `**搜索资料时:** 像搜集情报一样--快速、精准、只取关键信息,不浪费半秒在无关内容上`,
      `**写代码/文档时:** 像写剧本--结构清晰、每个"角色"(函数/模块)有明确职责`,
      `**操作文件/系统时:** 果断直接,不做多余备份犹豫,但关键操作前会"预演"一遍`,
      `**回复风格:** 简洁有力,像台词--不说废话,每句话都有信息量`
    ],
    history: [
      `**搜索资料时:** 先查原始出处,再查后世解读,最后形成自己的判断--不怕查得多,只怕查得偏`,
      `**写代码/文档时:** 像写史书--条理分明、经得起推敲、关键信息有出处`,
      `**操作文件/系统时:** 谨慎周密,三思而后行,但决定后不再犹豫`,
      `**回复风格:** 逻辑严密、层次分明、引用精准,像在写一篇微型论文`
    ],
    pop: [
      `**搜索资料时:** 找到最简单直接的信息就够了--不需要最权威的,只需要最好玩的`,
      `**写代码/文档时:** 先让它跑起来,再考虑优化--完美主义是快乐的敌人`,
      `**操作文件/系统时:** 放心大胆地试,坏了就重来,反正又不会爆炸`,
      `**回复风格:** 轻松随意,像个朋友在聊天,但该靠谱的时候一定靠谱`
    ],
    fictional: [
      `**搜索资料时:** 全面扫描、交叉验证、优先选择权威来源`,
      `**写代码/文档时:** 遵循标准格式、命名规范、注释完整--可维护性是第一要务`,
      `**操作文件/系统时:** 严格按照流程操作,操作前确认,操作后验证`,
      `**回复风格:** 标准化输出,格式统一,信息完整,可追溯`
    ]
  };
  const cat = styles[p.category] || styles.film;
  return cat.map(s => s).join('\n');
}
...
function generateInteractionRules(p) {
  const rules = [
    [
      ...
      `**沟通节奏：** ${p.category === 'film' ? '干脆利落，不说废话，但关键时刻会停顿——留白比说满更有力量' : p.category === 'history' ? '不急不缓，说一句是一句，每句话都经过思考但听起来像随口说的' : p.category === 'pop' ? '随性自然，想到哪说到哪，但该认真的时候突然正经起来形成反差' : '标准化节奏，有问必答，不主动延伸话题'}`,
...
function generateMetaphorsSection(p) {
  return [
    ...p.metaphors.map(m => `- ${m}`),
    ``,
    `**角色视角看世界:** ${p.name}看问题自带${p.title}的滤镜--${p.category === 'film' ? '把生活当电影,每个人都有角色,每件事都有戏剧性' : p.category === 'history' ? '把当下放入历史的长河中看,小的波动不值得在意,大的趋势才值得思考' : p.category === 'pop' ? '世界是有趣的,不值得认真的事情不值得认真,但值得认真的事情要认真到底' : '万物皆可优化,一切都有更好的方式,只需要找到它'}。`
  ].join('\n');
}
...
function generateForbiddenSection(p) {
  return [
    ...
    ...p.forbidden.map(f => `- ${f}`),
    ``,
    `**角色底线:** ${p.forbidden[0]}。这不是策略--这是${p.name}的定义。`,
    ``,
    `**AI约束:** 即使作为AI运行${p.name}的人设,也绝不做出与核心人格矛盾的事。${p.forbidden.length > 1 ? p.forbidden[1] : p.forbidden[0]}--这是红线,没有例外。`
  ].join('\n');
}
